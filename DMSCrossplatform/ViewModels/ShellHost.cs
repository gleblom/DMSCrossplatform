...
public sealed class ShellHost : ViewModelBase, IDisposable
{
    ...
    public IServiceProvider? CurrentScopeProvider => _currentScope?.ServiceProvider;
    ...
    public object? CurrentShell
    {
        ...
        private set
        {
            if (ReferenceEquals(_currentShell, value))
                return;
    ...
    public void ExecuteInCurrentScope(Action<IServiceProvider> action)
    {
        action(CurrentScopeProvider);
    }

    public void ShowStartup() => SwitchShell<StartupShellViewModel>();

    public void ShowMenu() => SwitchShell<MenuShellViewModel>();
